...
export default function AdminCDList() {
  ...
  const fetchCDs = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/cd`, {
  ...
  const handleDelete = async (cdId) => {
    ...
    try {
      ...
      const token = await getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/cd/${cdId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleClose = async (cdId) => {
    ...
    try {
      ...
      const token = await getToken();
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/cd/${cdId}/close`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSave = async (cdId) => {
    try {
      ...
      const token = await getToken();
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/cd/${cdId}`,
        editData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
