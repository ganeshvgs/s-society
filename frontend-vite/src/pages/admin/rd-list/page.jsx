...
export default function AdminRDTable() {
  ...
  const fetchRDs = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/rd`, {
  ...
  const handleDelete = async (rdId) => {
    ...
    try {
      ...
      const token = await getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/rd/${rdId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSave = async (rdId) => {
    ...
    try {
      ...
      const token = await getToken();
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/rd/${rdId}`,
        editForm,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleClose = async (rdId) => {
    ...
    try {
      ...
      const token = await getToken();
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/rd/close/${rdId}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
