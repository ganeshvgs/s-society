...
export default function AdminFDTable() {
  ...
  const fetchFDs = async () => {
    try {
      ...
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/fd`, {
        ...
        res.data.map(async (fd) => {
          try {
            const wRes = await axios.get(
              `${import.meta.env.VITE_API_URL}/api/fd/${fd._id}/withdrawals`,
              { headers: { Authorization: `Bearer ${token}` } }
            );
  ...
  const handleDelete = async (fdId) => {
    ...
    try {
      const token = await getToken();
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/fd/${fdId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSave = async (fdId) => {
    try {
      ...
      const token = await getToken();
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/fd/${fdId}`,
        editForm,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setEditingId(null);
  ...
  const handleClose = async (fdId) => {
    ...
    try {
      const token = await getToken();
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/fd/${fdId}/close`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
